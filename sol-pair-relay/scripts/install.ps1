...
$sourceSkill = Join-Path $packageRoot "skills\sol-pair-relay"
...
if ($conflicts.Count -gt 0) {
    throw (
        ...
        "Sol Pair Relay intentionally shares tm_* profile names with Poor Relay; " +
        "uninstall the active package before switching:" +
        [Environment]::NewLine +
        ($conflicts -join [Environment]::NewLine)
    )
}

if (-not $PSCmdlet.ShouldProcess($userProfilePath, "Install Sol Pair Relay user Skill and Agent profiles")) {
    return
}
