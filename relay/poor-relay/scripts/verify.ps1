...
if ($Installed) {
    $validatorPath = Join-Path $userProfilePath ".agents\skills\poor-relay\scripts\validate_poor_relay.py"
}
else {
    $validatorPath = Join-Path $packageRoot "skills\poor-relay\scripts\validate_poor_relay.py"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Poor Relay validation failed with exit code $LASTEXITCODE."
}

Write-Host "[OK] PowerShell scripts parsed successfully."
if ($Installed) {
    Write-Host "[OK] Installed Poor Relay files passed static validation."
}
else {
    Write-Host "[OK] Poor Relay source package passed static validation."
}
