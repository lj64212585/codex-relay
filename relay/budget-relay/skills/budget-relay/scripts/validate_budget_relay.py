...
EXPECTED_AGENTS = {
    "tm_planner.toml": ("tm_planner", "gpt-6-astra", "xhigh", "workspace-write"),
    ...
    "tm_integrator.toml": ("tm_integrator", "gpt-6-astra", "xhigh", "workspace-write"),
}
...
def locate_agent_dir() -> tuple[Path, bool]:
    ...
    if PACKAGE_ROOT.name not in (".codex", ".agents") and source_agents.is_dir():
        return source_agents, True

    installed_agents = SKILL_DIR.parents[2] / ".codex" / "agents"
...
def validate_skill() -> None:
    ...
    require('display_name: "Budget Relay"' in yaml_text, "openai.yaml display name mismatch")
    require("$budget-relay" in yaml_text, "default prompt must mention $budget-relay")
