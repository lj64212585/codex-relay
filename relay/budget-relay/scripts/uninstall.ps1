...
$skillParent = Join-Path $userProfilePath ".codex\skills"
$agentParent = Join-Path $userProfilePath ".codex\agents"
$targetSkill = Join-Path $skillParent "budget-relay"
$agentNames = @(
    "tm_planner.toml",
    "tm_explorer.toml",
    "tm_executor.toml",
...
if ($installedTargets.Count -eq 0) {
    Write-Host "[OK] Budget Relay is not installed for this user."
    return
}

if (-not $PSCmdlet.ShouldProcess(
    ($installedTargets -join ", "),
    "Remove only the Budget Relay Skill and five named Agent profiles"
)) {
    return
}
...
Write-Host "[OK] Removed Budget Relay user Skill and Agent profiles."
Write-Host "[NOTE] Parent .codex directory were preserved."
