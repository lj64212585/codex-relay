...
if ($Installed) {
    $validatorPath = Join-Path $userProfilePath ".codex\skills\budget-relay\scripts\validate_budget_relay.py"
}
else {
    $validatorPath = Join-Path $packageRoot "skills\budget-relay\scripts\validate_budget_relay.py"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Budget Relay validation failed with exit code $LASTEXITCODE."
}

Write-Host "[OK] PowerShell scripts parsed successfully."
if ($Installed) {
    Write-Host "[OK] Installed Budget Relay files passed static validation."
}
else {
    Write-Host "[OK] Budget Relay source package passed static validation."
}
