> 预算优先模式：适合行为已经确定的有界任务。高不确定性在实施前直接交给技术集成角色；需要持续产品或架构判断的整体任务应转回由能力足够的父会话主导。
...
<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="Budget Relay：在单个 Codex 任务中按成本和风险分层调度 Luna、Terra 与 Astra">
</p>

<p align="center"><strong>一次任务完成到底。让 Luna 承担大多数工作，只在判断真正值钱时升级到 Terra 或 Astra。</strong></p>

<p align="center">
  <a href="#为什么是-budget-relay">为什么</a> ·
  <a href="#调度策略">调度策略</a> ·
  <a href="#快速开始">快速开始</a> ·
...
Budget Relay 是一个面向 **Codex Desk 单会话**的低成本多 Agent 调度包。它让 Coordinator 保留用户意图、权限和任务生命周期，把高上下文调查与常规实现交给 Luna，并把更昂贵的独立判断、架构规划和最终集成限制在确有价值的节点。
...
| **Astra 按需介入** | 多步骤规划、失败升级、最终技术集成 | `tm_planner` · `tm_integrator` |

> 推荐由 Luna Max 担任 Coordinator。Budget Relay 不覆盖当前 task 的有效模型、权限或仓库规则。
...
- **仓库治理优先**：若用户或仓库要求 OpenSpec、特定 Git 流程或其他规则，继续服从；Budget Relay 不删除或绕过这些文件。
...
卸载脚本只移除 Budget Relay Skill 和五个同名 Agent profile，并保留父级 `.codex` 目录。

## 来源与许可证

Budget Relay 的文本和脚本为重新编写，工作流设计吸收了 `codex-team-mode`、`wshobson/agents`、`agency-agents` 与 `Superpowers` 的部分思想。具体来源文件、版权与改编边界见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
