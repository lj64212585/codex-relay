> Budget-priority mode for bounded tasks with settled behavior. Route high uncertainty to technical integration before implementation. Whole tasks needing continuous product or architecture judgment belong with a sufficiently capable parent.
...
<p align="center">
  <img src="./assets/readme/hero.en.svg" width="100%" alt="Budget Relay: cost- and risk-aware orchestration of Luna, Terra, and Astra within one Codex task">
</p>

<p align="center"><strong>One task, end to end. Let Luna carry the bulk of the work, and escalate to Terra or Astra only when the judgment is worth the cost.</strong></p>

<p align="center">
  <a href="#why-budget-relay">Why</a> ·
...
| **Astra joins on demand** | Multi-step planning, failed-task escalation, and final technical integration | `tm_planner` · `tm_integrator` |

> Luna Max is the recommended Coordinator. Budget Relay does not override the current task's effective model, permissions, or repository rules.
...
From the `budget-relay` directory, verify the source package before installing it:
...
```text
%USERPROFILE%\.codex\skills\budget-relay\
...
```text
Use $budget-relay to complete this multi-step project task.
```
...
│   └── scripts/validate_budget_relay.py
├── scripts/
│   ├── install.ps1
│   ├── uninstall.ps1
│   └── verify.ps1
...
- **Repository governance wins:** If the user or repository requires OpenSpec, a particular Git workflow, or another governance process, those rules still apply. Budget Relay does not remove or bypass them.
...
The uninstaller removes only the Budget Relay Skill and the five same-named Agent profiles. It preserves the parent `.codex` directory.

## Provenance and license

Budget Relay's text and scripts were independently written. Its workflow design adapts selected ideas from `codex-team-mode`, `wshobson/agents`, `agency-agents`, and `Superpowers`. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for the specific upstream files, copyright notices, and adaptation boundaries.
