...
- Relay 类型使用纵向排列的横向长条；每个名称下方说明适用任务与职责边界，不显示未经实测的质量或成本百分比。每条左侧的“详情”按钮独立于单选区域，打开与当前界面语言对应的 README 抽屉，不会改变已选择的 Relay。
...
- 单独移除调度时同样会先写入 .relay-installer-backups 可恢复备份；只移动能够归属到配置中 Relay 的 Skill 目录和 Agent 文件，不删除未知文件，也不清理 .codex/skills 或 .codex/agents 父目录。
- 如果同名 Agent 文件无法归属到任一已配置 Relay，安装器会停止，不会把自定义文件当作 Relay 自动删除。

多个调度 Skill 同时存在时，隐式路由规则可能竞争同一个任务。因此本安装器把“同一范围只保留一种主调度”作为默认安全边界。
...
仓库级 Win64 打包入口会从 `packaging/version.txt` 读取版本号，把桌面窗口、网页工具、配置与当前配置列出的三套 Relay 全部固化进一个 EXE，并在构建后检查桌面入口、包内内容、内置版本和 Windows 版本元数据：
...
版本文件只写一行 `MAJOR.MINOR.PATCH` 或 `MAJOR.MINOR.PATCH.BUILD`，例如 `1.0.0`；每段必须在 0—65535 之间。它以及 `packaging/` 下的其他内容都由该目录的 `.gitignore` 忽略，不进入 Git。构建产物位于 `packaging/out/win64/<版本号>/`，其中包含 `relay-installer-v<版本号>.exe` 和 `SHA256SUMS.txt`。校验文件记录完整 EXE 的 SHA-256 与文件名，在产物验证通过后生成；版本同时显示在窗口标题与 Windows 文件属性中。
...
.\tools\relay-installer\build.ps1 -Mode onefile -OutputDirectory packaging\out\win64 -RequireWin64 -VersionFile packaging\version.txt -VerifyPackage -VersionedOutput -WriteChecksums
~~~
...
配置中的 `retiredRelays` 仅保存旧安装路径及换行归一化 SHA-256 指纹，不包含可安装的旧模式。预检将旧版列为冲突；确认切换后先备份再安装，失败回滚。自定义或未知旧文件会阻止自动操作。识别范围是本次迁移前版本，其他版本需人工核对。源码更名不会修改已安装到用户目录的文件；打包需重新构建。

## Skill 路径

新安装统一写入 `<目标根目录>/.codex/skills/<relay>`，Agent 写入 `.codex/agents`。旧 `.agents/skills` 路径仅用于已知 Relay 的指纹识别与备份迁移；自定义或修改过的旧文件会阻止迁移，不会静默删除。已有安装请使用安装器完成迁移，独立复制命令与 Budget Relay 脚本只处理新路径。

兼容性：当前 [Codex 官方文档](https://learn.chatgpt.com/docs/build-skills) 列出的自动发现目录仍为 `.agents/skills`；本项目按要求使用 `.codex/skills`，需在所用 Codex 版本的新任务中确认技能可见性，文件校验不代表运行时发现已通过。
